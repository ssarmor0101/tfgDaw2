...
function useGames() {
  ...
  useEffect(() => {
    let cancelled = false
    fetch(API_ROUTES.GAMES.ALL, { headers: { Accept: 'application/json' } })
      .then(async r => {
        const json = await r.json() as { data: Juego[] }
        if (!cancelled) setGames(Array.isArray(json.data) ? json.data : [])
      })
      .catch(() => {})
...
function usePlayerScores(userId: number, gameId: number | null) {
  const [scores, setScores] = useState<ScoreEntry[]>([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    if (!gameId) { setScores([]); setError(null); return }
    let cancelled = false
    setLoading(true)
    setError(null)
    fetch(API_ROUTES.SCORES.BY_USER_AND_GAME(userId, gameId), {
...
function MedalIcon({ rank }: { rank: number }) {
  ...
  return <span className="text-xs text-arcade-muted tabular-nums">{rank}</span>
}
  ...
  return (
    <div className="animate-pulse">
      <div className="grid grid-cols-[48px_1fr_1fr] gap-4 px-5 py-3 border-b border-arcade-border">
        {[1, 2, 3].map(i => <div key={i} className="h-3 bg-white/10 rounded w-16" />)}
      </div>
...
function Pagination({ page, lastPage, total, onPage }: PaginationProps) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 px-5 py-4 border-t border-arcade-border">
      <p className="text-xs text-arcade-muted">{total} partida{total !== 1 ? 's' : ''} en total</p>
...
export function PlayerPuntuaciones() {
  ...
  // Use gameId from navigation state as initial value
  const [selectedGameId, setSelectedGameId] = useState<number | null>(state.gameId ?? null)

  const { scores, loading: scoresLoading, error } = usePlayerScores(playerId, selectedGameId)

  // Derive friend name: from state, or from first score's user data
  const friendName = state.friendName
    ?? scores[0]?.user?.name
    ?? `Jugador #${playerId}`

  // Derive selected game name
  ...
  // Table — sorted by score descending (highest first)
  ...
  const pageItems = useMemo(
    () => byScore.slice((page - 1) * PER_PAGE, page * PER_PAGE),
    [byScore, page]
  )
  ...
  const handleGameChange = (id: string) => {
    setSelectedGameId(id ? Number(id) : null)
    // Update nav state so back navigation keeps context
    navigate(location.pathname, {
      replace: true,
      state: { ...state, gameId: id ? Number(id) : undefined },
    })
  }
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-8">
      ...
      <nav className="flex items-center gap-2 text-xs text-arcade-muted uppercase tracking-widest">
        <Link to="/amigos" className="hover:text-white transition-colors">Amigos</Link>
        <span>/</span>
        <span className="text-white">{friendName}</span>
        <span>/</span>
        <span className="text-white">Puntuaciones</span>
      </nav>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end gap-6 justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold text-white">
            Puntuaciones de&nbsp;
            <span className="text-arcade-cyan">{friendName}</span>
          </h1>
          {selectedGame && (
            <p className="text-arcade-muted text-sm mt-1">
              en <span className="text-white font-medium">{selectedGame.name}</span>
            </p>
          )}
        </div>
        ...
        <div className="shrink-0 w-full sm:w-64">
          ...
          {gamesLoading ? (
            <div className="flex items-center gap-2 px-3 py-2 bg-arcade-surface border border-arcade-border rounded-lg">
              <LoadingSpinner />
              <span className="text-xs text-arcade-muted">Cargando juegos…</span>
            </div>
          ) : (
            <select
              value={selectedGameId ?? ''}
              onChange={e => handleGameChange(e.target.value)}
              className="w-full px-3 py-2.5 bg-arcade-surface border border-arcade-border rounded-lg text-sm text-white focus:outline-none focus:border-arcade-purple transition-colors"
            >
      ...
      {!selectedGameId && (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center
          ...
          <div>
            <p className="text-white font-semibold text-sm uppercase tracking-widest">Selecciona un juego</p>
            <p className="text-arcade-muted text-xs mt-1.5 max-w-[260px]">
              Elige un juego del selector para ver las puntuaciones de {friendName}.
            </p>
          </div>
        </div>
      )}
      ...
      {/* Stats */}
      {selectedGameId && !scoresLoading && !error && total > 0 && (
        ...
        error ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center
            ...
            <div>
              ...
              <p className="text-arcade-muted text-xs mt-1.5 max-w-[260px]">
                {friendName} aún no ha enviado ninguna puntuación para este juego.
              </p>
            </div>
          </div>
        ) : (
